        ...
        <main>
            ...
            <div class="button__container">
                <div class="button__block center">
                    <button :key="button.id" v-for="(button) in allTags" @click="filterByTag(button)" class="button">
                        {{ button.title }}
                    </button>
                </div>
            </div>
            <div class="card-block center">
                <CardsBlock v-for="(card) in currentProductList" :key="card.id" :cardTitle="cardTitle"
                    :cardSubtitle="cardSubtitle" :cardImg="cardImg" :tag="tag" :card="card" />
            </div>
            <div id="pages" class="pagination">
                <router-link v-for="page in 3" :to="`/ProjectPage/${page}`" :key="page" class="container">
